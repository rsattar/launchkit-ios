//
//  LK_SSZipArchive.m (formerly SSZipArchive.m, vendored by LaunchKit)
//  SSZipArchive
//
...
@interface LK_SSZipArchive ()
+ (NSDate *)_dateWithMSDOSFormat:(UInt32)msdosDateTime;
@end

@implementation LK_SSZipArchive
{
	NSString *_path;
	NSString *_filename;
    zipFile _zip;
...
+ (BOOL)unzipFileAtPath:(NSString *)path
		  toDestination:(NSString *)destination
			  overwrite:(BOOL)overwrite
			   password:(NSString *)password
				  error:(NSError **)error
			   delegate:(id<LK_SSZipArchiveDelegate>)delegate
		progressHandler:(void (^)(NSString *entry, unz_file_info zipInfo, long entryNumber, long total))progressHandler
	  completionHandler:(void (^)(NSString *path, BOOL succeeded, NSError *error))completionHandler
{
	// Begin opening
...
{
	BOOL success = NO;
	LK_SSZipArchive *zipArchive = [[LK_SSZipArchive alloc] initWithPath:path];
	if ([zipArchive open]) {
